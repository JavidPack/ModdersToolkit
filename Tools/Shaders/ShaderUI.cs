...
");
		...
		private void WatchShader_OnSelectedChanged() {
			...
			if (watchQuickShaderCheckbox.Selected) {
				...
				quickShaderWatcher.Path = cacheFolder; // ModSourcePath + Path.DirectorySeparatorChar + selectedMod.Name + Path.DirectorySeparatorChar;
				/* Watch for changes in LastAccess and LastWrite times, and
				   the renaming of files or directories. */
				quickShaderWatcher.NotifyFilter = NotifyFilters.LastWrite | NotifyFilters.FileName;
				// Only watch text files.
				quickShaderWatcher.Filter = shaderFilename;
				quickShaderWatcher.IncludeSubdirectories = false;
